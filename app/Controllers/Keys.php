    ...
    public function resetDeviceKey()
{
    ...
    //delete exp key
public function deleteEXPKeys() {
    $user = $this->user;
    $model = $this->model;
    $usernameexp = $user->username;

    $model->where('registrator', $usernameexp)->where('expired_date <', date('Y-m-d H:i:s'))->delete();
...
private function edit_key_action()
    ...
    private function generate_action()
    {
        ...
        $chanlicense = $this->request->getPost('user_key');
        
  if (empty($chanlicense)) {
        $license = random_string('alnum',10);
        $chanlicense = $game . "-" . $license;
    } else {
        $chanlicense = $chanlicense;
    }
 ...
 // * reseller reduce saldo
             ...
             $license = 'YMYCheat-'. $drtn. 'H-'. random_string('alnum', 10);
